...
export default function Home() {
  ...
  const handleStartApp = () => {
    ...
    if (player.localPlayer.audioRef.current) {
      player.localPlayer.audioRef.current.play().then(() => {
      ...
      {needsInteraction && (
        <div className="fixed inset-0 z-[100] bg-background/80 backdrop-blur-md flex flex-col items-center justify-center p-6 text-center">
          <div className="max-w-md space-y-6">
            ...
            <p className="text-muted-foreground">To enable high-quality audio and YouTube streaming, please click the button below to start the player.</p>
            <Button size="lg" onClick={handleStartApp} className="gap-2 px-8 py-6 text-lg rounded-full shadow-xl hover:scale-105 transition-transform">
      ...
      <div className="relative z-10 flex flex-col h-full w-full">
        ...
        <main className="flex-1 grid grid-cols-1 lg:grid-cols-[320px_1fr_320px] gap-6 p-6 overflow-hidden z-10">
          ...
          <aside className="hidden lg:flex flex-col bg-card/50 backdrop-blur rounded-xl border border-border p-4 overflow-hidden">
            <h3 className="font-semibold mb-3 flex items-center gap-2 flex-none"><Music className="w-4 h-4" /> Queue ({player.queue.length})</h3>
            <div className="flex-1 overflow-y-auto scrollbar-thin space-y-2 pr-1">
              {player.queue.map((track) => (
                <div key={track.id} className={`group p-3 rounded-lg border transition-all cursor-pointer flex items-center gap-2 ${player.currentTrack?.id === track.id ? 'bg-primary/20 border-primary/50' : 'bg-background/50 hover:bg-foreground/5 border-border/50'}`} onClick={() => player.playTrack(track)}>
                  <div className="relative w-10 h-10 flex-none">
                    {track.albumCover ? <img src={track.albumCover} className="w-full h-full object-cover rounded" /> : <div className="w-full h-full bg-muted rounded flex items-center justify-center"><Music className="w-5 h-5 opacity-20" /></div>}
                    {track.sourceType === 'youtube' && <div className="absolute -bottom-1 -right-1 w-4 h-4 bg-background rounded-full flex items-center justify-center border border-border"><Youtube className="w-2.5 h-2.5 text-red-500" /></div>}
                  </div>
                  <div className="flex-1 min-w-0"><p className="text-sm font-medium truncate">{track.title}</p><p className="text-xs text-muted-foreground truncate">{track.artist}</p></div>
                  <Button size="icon" variant="ghost" className="w-6 h-6 opacity-0 group-hover:opacity-100 flex-none" onClick={(e) => { e.stopPropagation(); player.removeFromQueue(track.id); }}><X className="w-3 h-3" /></Button>
                </div>
              ))}
            </div>
          </aside>
          ...
          <section className="flex flex-col items-center justify-center gap-8 overflow-hidden relative z-20">
            <div className="w-full max-w-md aspect-square bg-card/30 backdrop-blur-md rounded-2xl border border-border/50 shadow-2xl flex items-center justify-center overflow-hidden relative group flex-none">
              {/* YouTube Player - Visible but small to satisfy Error 150 */}
              <div 
                className={`absolute inset-0 z-10 bg-black transition-opacity duration-500 ${player.currentTrack?.sourceType === 'youtube' ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
              >
              ...
              {!albumCoverUrl && player.currentTrack?.sourceType !== 'youtube' && (
                <div className="flex flex-col items-center gap-4 text-muted-foreground">
                  <Music className="w-20 h-20 opacity-20" /><p className="text-sm font-medium opacity-50">No track playing</p>
                </div>
              )}
          ...
          <aside className="hidden lg:flex flex-col bg-card/50 backdrop-blur rounded-xl border border-border p-6 overflow-hidden">
            <SmartEqualizer canUse={player.canUseEqualizer} settings={player.localPlayer.equalizerSettings} onBandChange={player.localPlayer.updateEqualizerBand} onIntensityChange={player.localPlayer.updateEqualizerIntensity} onToggle={player.localPlayer.toggleEqualizer} />
          </aside>
